...
public:
	...
	boost::optional<value_type> try_pop(){
		return try_pop_for(std::chrono::nanoseconds{0});
	}

	template<typename REP,typename RATIO>
	boost::optional<value_type> try_pop_for(std::chrono::duration<REP,RATIO> timespan){
		ulock lock{this->lock_};
		boost::optional<value_type> o{};
		if(notEmpty.wait_for(lock,timespan,[this]{return !doempty();})){
			o = std::move(this->dopop());
			if(o){
				notFull.notify_one();
			}
		}
