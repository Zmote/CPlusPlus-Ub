...
int charMapper(char const inputChar){
switch(inputChar){
	case 'e': return 10;
	case 'r': return 11;
	case 'o': return 12;
...
void printLargeNumber(std::ostream &o, std::istream &in, int const scale){
	std::string givenNumber{};
	getline(in,givenNumber);
	for(int i = 0; i < 5; i++){
		if(i == 0 || i == 2 || i == 4){
...
void printSingleLNumber(std::ostream &o, std::istream &i){
	int in;
	i >> in;
	if(!i){
		i.clear();
