...
public:
	...
	BoundedBuffer(BoundedBuffer const & elem){
		//the entire if(this!= &elem) etc. checks can be skipped, because
		//this is the copy-ctor, ie this isn't initialized in any way
		//Note: Only section starting from "auto elembuffer" and little above needed
		if(this != &elem){
			if(size() > 0){
				clear();
			}
